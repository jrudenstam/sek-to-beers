 ...
 (function (root, factory) {
	...
	return {
		...
		jsonpCallback:function( callback ){
			...
			return encodeURIComponent('window.cbs['+(window.cbs.length-1)+']');
		},
		...
		/*
		/* params: url(string), data(object), callback(function)
		/* Takes URL to a JSON, query strings from data-object, and callback is the function which should run after request is done
		*/
		larsjsonp: function( url, data, callback ) {

			if(tempScript) return; // If data parsing is already in action, abort.

			var data = data || {},
				src = url + (url.indexOf( '?' ) + 1 ? '&' : '?'),
				head = document.getElementsByTagName( 'head' )[0],
				tempScript = document.createElement( 'script' ),
				internalCallback = 'larsjsonpComplete',
				params = [];

			window.jsonpCallback = callback;

			for( var parameter in data ) {
				params.push( parameter + '=' + encodeURIComponent( data[parameter] ));
			}

			src += params.join( '&' ) + '&callback=helper.' + internalCallback;
		...
		larsjsonpComplete: function( data ) {
			document.getElementsByTagName( 'head' )[0].removeChild( document.getElementById( 'tempscript' ) );

			jsonpCallback(data);
			window.jsonpCallback = false;
		},
